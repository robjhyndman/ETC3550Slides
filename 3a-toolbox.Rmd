...
food %>% autoplot(sqrt(Turnover)) +
  labs(y = "Square root turnover", x = NULL)
```
...
food %>% autoplot(Turnover^(1/3)) +
  labs(y = "Cube root turnover", x = NULL)
```
...
food %>% autoplot(-1/Turnover) + 
  labs(y = "Inverse turnover", x = NULL)
```
...
food %>% autoplot(box_cox(Turnover, 1/3)) + 
  labs(y = "Box-Cox transformed turnover", x = NULL)
```
...
\begin{block}{}
\begin{tabular}{ll}
\bf Mean: & $\hat{y}_{T+h} \sim N(\bar{y}, (1 + 1/T)\hat{\sigma}^2)$\\[0.2cm]
\bf Naïve: & $\hat{y}_{T+h} \sim N(\bar{y}, h\hat{\sigma}^2)$\\[0.2cm]
\bf Seasonal naïve: & $\hat{y}_{T+h} \sim N(y_{T+h-m(k+1)}, (k+1)\hat{\sigma}^2)$\\[0.2cm]
\bf Drift: & $\hat{y}_{T+h} \sim N(y_T + \frac{h}{T-1}(y_T - y_1),h\frac{T+h}{T}\hat{\sigma}^2)$.
\end{tabular}
\end{block}
