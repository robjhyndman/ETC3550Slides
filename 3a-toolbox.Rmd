...
bricks %>% 
  ...
  forecast(h = "5 years") %>% 
  autoplot(filter(bricks, year(Quarter) > 1990), level = NULL) + 
  geom_point(data = slice(bricks, (n()-4):n()), colour = "blue") + 
  ggtitle("Clay brick production in Australia")
```
...
\alert{Code for previous graph}\fontsize{9}{10}\sf
...
```{r googforecasts, echo=TRUE}
forecast(fit, h = 1) %>% select(-.model, -Symbol) %>% 
