...
## Estimation and stationarity
\fontsize{13}{14}\sf

  * All variables in the model must be stationary before estimation.
  ...
  * In R, we can specify an ARIMA($p,d,q$) for the errors, and $d$ levels of differencing will be applied to all variables ($y, x_{1,t},\dots,x_{k,t}$).
...
```{r usconsump, fig.height=6, fig.width=8.5, echo=FALSE}
us_change %>%
...
fit <- insurance %>%
  ...
  # Estimate models
  model(
    ARIMA(Quotes ~ pdq(d = 0) + TVadvert),
    ARIMA(Quotes ~ pdq(d = 0) + TVadvert + lag(TVadverts)),
    ARIMA(Quotes ~ pdq(d = 0) + TVadvert + lag(TVadverts) +
            lag(TVadvert, 2)),
    ARIMA(Quotes ~ pdq(d = 0) + TVadverts + lag(TVadverts) +
            lag(TVadvert, 2) + lag(TVadverts, 3))
  )
```
