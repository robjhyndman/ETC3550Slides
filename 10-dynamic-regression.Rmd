...
vic_elec_daily <- aus_elec %>% 
  filter(State == "Victoria", year(Time) == 2014) %>% 
...
us_change %>%
  ...
  ggplot(aes(y=value, x=Time, group=variable)) +
...
```{r, echo=TRUE}
fit <- vic_elec_daily %>% 
  model(ARIMA(Demand ~ Temperature + Temperature^2 + 
                (Day_Type=="Weekday")))
report(fit)
```
...
coef <- set_names(tidy(fit_deterministic)$estimate, tidy(fit_deterministic)$term)
phi1 <- coef['ar1']
phi2 <- coef['ar2']
intercept <- coef['intercept']
slope <- coef['trend']
sigma2 <- glance(fit_deterministic)$sigma^2
```
...
coef <- set_names(tidy(fit_stochastic)$estimate, tidy(fit_stochastic)$term)
drift <- coef['constant']
theta1 <- coef['ma1']
sigma2 <- glance(fit_stochastic)$sigma^2
```
...
```{r, echo = FALSE}
glance(fit) %>% 
  select(.model, sigma, logLik, AIC, AICc, BIC) %>% 
  knitr::kable()
```
...
(calls <- read_tsv("http://robjhyndman.com/data/callcenter.txt") %>% 
  gather("date", "volume", -X1) %>% transmute(
    ...
    datetime = as.POSIXct(date) + time, volume) %>% 
...
```{r, echo = FALSE}
glance(fit) %>% 
  transmute(`Lag order` = 0:3, sigma, logLik, AIC, AICc, BIC) %>% 
  knitr::kable()
```
...
# Store coefficients
coef <- set_names(tidy(fit)$estimate, tidy(fit)$term)
phi1 <- coef['ar1']
phi2 <- coef['ar2']
phi3 <- coef['ar3']
intercept <- coef['intercept']
gamma0 <- coef['xreg.TV.advert']
gamma1 <- coef['xreg.lag(TV.advert)']
```
