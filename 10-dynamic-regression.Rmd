...
elec_daily <- tsibbledata::aus_elec %>%
...
```{r , echo=TRUE, fig.height=3.7, eval=FALSE}
augment(fit, type='regression') %>%
  gg_tsdisplay(.resid)
```
...
```{r , echo=TRUE, fig.height=3.7}
Box.test(augment(fit)$.resid, fitdf=5, lag=12)
```

## US personal consumption and income
\fontsize{9}{12}\sf

```{r usconsump3, echo=TRUE, fig.height=3., eval=FALSE}
fcast <- forecast(fit,
