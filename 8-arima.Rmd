...
goog <- tsibble(Time = seq(ymd("2013-02-25"), length.out = 1000, by = 1),
                Price = fpp2::goog, index = Time)

uschange <- read_csv("data/uschange.csv") %>% 
  mutate(Time = yearquarter(Time)) %>% 
  as_tsibble(index = Time)

euretail <- as_tsibble(fpp2::euretail)
...
```{r}
dj %>% autoplot(value) + ylab("Dow Jones Index") + xlab("Day")
```

## Stationary?

```{r}
dj %>% autoplot(difference(value)) + ylab("Change in Dow Jones Index") + xlab("Day")
```
...
## Stationary?
```{r}
as_tsibble(fma::pigs) %>% filter_index("1990" ~ .) %>% 
  autoplot(value/1e3) + xlab("Year") + ylab("thousands") +
...
```{r}
as_tsibble(lynx) %>% autoplot(value) + xlab("Year") + ylab("Number trapped") +
...
```{r}
ausbeer %>% filter_index("1992" ~ .) %>% 
  autoplot(Production) + xlab("Year") + ylab("megalitres") +
...
```{r}
dj %>% ACF(value) %>% autoplot()
```

## Example: Dow-Jones index

```{r}
dj %>% autoplot(difference(value)) + 
...
```{r}
dj %>% ACF(difference(value)) %>% autoplot()
```
...
```{r, echo=TRUE}
fit <- uschange %>% model(arima = ARIMA(Consumption))
report(fit)
```
...
### ARIMA(2,0,2) model:
\centerline{$
  y_t = c + `r format(NA, nsmall=3, digits=3)`y_{t-1}
          `r format(NA, nsmall=3, digits=3)` y_{t-2}
          `r format(NA, nsmall=3, digits=3)` \varepsilon_{t-1}
          + `r format(NA, nsmall=3, digits=3)` \varepsilon_{t-2}
          + \varepsilon_{t},
$}
where $c= `r format(NA, nsmall=3, digits=3)` \times (1 - `r format(NA, nsmall=3, digits=3)` + `r format(NA, nsmall=3, digits=3)`) = `r format(NA, nsmall=3, digits=3)`$
and $\varepsilon_t$ is white noise with a standard deviation of $`r format(NA, nsmall=3, digits=3)` = \sqrt{`r format(NA, nsmall=3, digits=3)`}$.
...
```{r, echo=TRUE, fig.height=4}
fit %>% forecast(h=10) %>% autoplot(tail(uschange,80))
```
...
```{r}
uschange %>%
  autoplot(difference(Consumption)) +
    xlab("Year") +
    ylab("Quarterly percentage change") +
    ggtitle("US consumption")
```
...
```{r usconsumptionacf}
p1 <- uschange %>% ACF(Consumption) %>% autoplot()
p2 <- uschange %>% PACF(Consumption) %>% autoplot()
gridExtra::grid.arrange(p1,p2,nrow=1)
```
...
```{r, echo=TRUE, fig.height=4}
internet <- as_tsibble(fma::internet)
internet %>% ggtsdisplay(value)
```

## Choosing your own model

```{r, echo=TRUE, fig.height=4}
internet %>% ggtsdisplay(difference(value))
```
...
```{r, echo=TRUE, fig.height=4}
fit <- internet %>% model(arima = ARIMA(value ~ pdq(3, 1, 0)))
report(fit)
```
...
```{r, echo=TRUE, fig.height=4}
internet %>% model(ARIMA(value)) %>% report()
```
...
internet %>% model(ARIMA(value, stepwise = FALSE, approximation = FALSE)) %>% report()
```
...
```{r, echo=TRUE, fig.height=4}
fit %>% residuals %>% ggtsdisplay(.resid, plot_type = "hist")
```
...
```{r}
Box.test(residuals(fit)$.resid, fitdf = 3, lag = 10, type = "Lj")
```
...
```{r, echo=TRUE, fig.height=4}
fit %>% forecast(h = 10) %>% autoplot(internet)
```
...
dcmp %>% as_tsibble %>% 
  autoplot(seas_adjust) + xlab("Year") +
...
dcmp %>% ggtsdisplay(difference(seas_adjust))
```
...
```{r, echo=TRUE}
fit <- dcmp %>% model(arima = ARIMA(seas_adjust ~ pdq(3,1,1) + PDQ(0,0,0)))
report(fit)
```
...
```{r}
Box.test(residuals(fit)$.resid, lag = 24, fitdf = 4, type = "Lj")
```
...
```{r, echo=TRUE, fig.height=4}
euretail %>% ggtsdisplay(difference(value, 4))
```

## European quarterly retail trade

```{r, echo=TRUE, fig.height=3.8}
euretail %>% ggtsdisplay(value %>% difference(4) %>% difference(1))
```
...
```{r, echo=TRUE, fig.height=3.5}
fit <- euretail %>% model(arima = ARIMA(value ~ pdq(0,1,1) + PDQ(0,1,1)))
fit %>% residuals() %>% ggtsdisplay(plot_type = "hist")
```
...
```{r}
Box.test(residuals(fit)$.resid, lag = 8, fitdf = 2, type = "Lj")
```
...
aicc <- euretail %>% 
  model(
    mdl_1 = ARIMA(value ~ pdq(0,1,2) + PDQ(0,1,1)),
    mdl_2 = ARIMA(value ~ pdq(0,1,3) + PDQ(0,1,1))
  ) %>% 
  glance %>% 
  pull(AICc)
```
...
```{r}
fit <- euretail %>% model(ARIMA(value ~ pdq(0,1,3) + PDQ(0,1,1)))
```
...
```{r}
Box.test(residuals(fit)$.resid, lag = 8, fitdf = 4, type = "Lj")
```

## European quarterly retail trade

```{r, echo=TRUE, fig.height=4}
fit %>% forecast(h = "3 years") %>% autoplot(euretail)
```
...
```{r, echo=TRUE}
euretail %>% model(ARIMA(value)) %>% report
```
...
euretail %>% model(ARIMA(value, stepwise = FALSE, approximation = FALSE)) %>% report
```
...
## Cortecosteroid drug sales
```{r h02b}
h02 %>% ggtsdisplay(difference(log(Cost),12))
```
...
model_defs <- map(models, ~ ARIMA(log(Cost) ~ pdq(!!.[1], !!.[2], !!.[3]) + PDQ(!!.[4], !!.[5], !!.[6])))
...
fit <- h02 %>% model(best = ARIMA(log(Cost) ~ pdq(3,0,1) + PDQ(0,1,2)))
report(fit)
```
...
fit %>% residuals %>% ggtsdisplay(.resid, lag_max=36, plot_type = "hist")
```
...
```{r h02resb, fig.height=4, dependson='arimah02'}
Box.test(residuals(fit)$.resid, lag = 36, fitdf = 6, type = "Lj")
```
...
residuals(fit) %>% ggtsdisplay(.resid, lag_max = 36, plot_type = "hist")
```
...
```{r, dependson='h02auto'}
Box.test(residuals(fit)$.resid, lag = 36, fitdf = 5, type = "Lj")
```
...
```{r, dependson='h02tryharder'}
Box.test(residuals(fit)$.resid, lag=36, fitdf = 9, type = "Lj")
```
...
fit <- h02 %>% model(ARIMA(Cost ~ pdq(3,0,1) + PDQ(0,1,2)))
