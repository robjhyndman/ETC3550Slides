...
uschange %>% 
  gather("Measure", "Change", Production, Savings, Unemployment) %>% 
...
```{r, echo=TRUE}
fit_beer <- beer %>% 
  model(TSLM(Production ~ trend() + season()))
#summary(fit.beer)
```
## Beer production revisited
...
fourier_beer <- beer %>% 
  model(TSLM(Production ~ trend() + fourier(K=2)))
...
fit_lin <- marathon %>% model(TSLM(Minutes ~ trend()))
```
...
augment(fit_lin) %>%
  autoplot(Minutes) +
  geom_line(aes(y = .fitted, colour = "Linear trend")) +
  xlab("Year") +  ylab("Winning times in minutes") +
  guides(colour=guide_legend(title=" ")) +
...
residuals(fit_lin) %>% autoplot(.resid) +
...
```{r residPiecewise, message=FALSE, warning=FALSE, cache=TRUE, eval = FALSE}
...
# Spline trend
library(splines)
fit_splines <- marathon %>% 
  model(splines = TSLM(Minutes ~ ns(Year, df=6)))
fitted(fit_splines)
```
...
library(splines)
fc_splines <- fit_splines %>% forecast(h=10)
fit_piecewise <- fit_trends %>% select(piecewise)
fc_piecewise <- fc_trends %>% filter(.model == "piecewise")

marathon %>% 
  autoplot(Minutes) +
  geom_line(aes(y = .fitted, colour = .model), 
            data = bind_rows(fitted(fit_piecewise), fitted(fit_splines))) + 
  autolayer(fc_piecewise, alpha = 0.3) +
  autolayer(fc_splines, alpha = 0.3) +
  xlab("Year") +  ylab("Winning times in minutes") +
  ggtitle("Boston Marathon") +
  guides(colour=guide_legend(title=" "))
```
...
```{r, echo=TRUE, fig.height=3.3, eval = FALSE}
fc <- splinef(marathon)
autoplot(fc)
```
...
fit.beer <- beer2 %>% model(TSLM(Production ~ trend() + season()))
fcast <- forecast(fit.beer)
fcast %>% autoplot(beer2) +
...
fit_consBest <- uschange %>% 
  model(lm = TSLM(Consumption ~ Income + Savings + Unemployment))
up_future <- new_data(uschange, 4) %>% 
  mutate(Income = 1, Savings = 0.5, Unemployment = 0)
down_future <- new_data(uschange, 4) %>% 
  mutate(Income = -1, Savings = -0.5, Unemployment = 0)
