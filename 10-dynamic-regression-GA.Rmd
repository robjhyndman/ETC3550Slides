...
author: "Ch10. Dynamic Regression"
date: "OTexts.org/fpp3/"
classoption: aspectratio=169
titlepage: MRight1.jpg
titlecolor: MonashBlue
toc: true
output:
  binb::monash:
...
```{r usconsump2writemodel, echo=FALSE}
fit <- us_change %>% model(ARIMA(Consumption ~ Income))
report(fit)
```

\orange{Write down the equations for the fitted model.}
\vspace{3cm}

## US personal consumption and income
\fontsize{9}{10}\sf

```{r , echo=TRUE, fig.height=2.8}
 ...
 **\orange{Combine Fourier terms with ARIMA errors}**
...
glance(fit) %>% select(.model, sigma2, log_lik, AIC, AICc, BIC)
```
...
fit <- us_gasoline %>% model(ARIMA(Barrels ~ fourier(K = 6) + PDQ(0,0,0)))
report(fit)
```
...
calls %>% fill_gaps() %>%
  gg_season(volume, period = "day", alpha = 0.2) +
  guides(colour = FALSE)
```
...
calls_mdl <- calls %>% mutate(idx = row_number()) %>% update_tsibble(index = idx)
...
\[ y_t = \beta_0 + \beta_1 t + \eta_t \]
where $\eta_t$ is ARIMA process with $d=1$.\pause
...
aus_airpassengers %>% autoplot(Passengers) +
  ...
  autolayer(fit_deterministic %>% forecast(h = 20), colour = "#D55E00", level = 95, 
            alpha = 0.65) +
...
\orange{Sometimes a change in $x_t$ does not affect $y_t$ instantaneously}\pause
\begin{block}{}
\begin{itemize}
