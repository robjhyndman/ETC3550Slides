...
We design $\beta^*$ to weight slopes more.

$$\beta^*_t = \beta^*(\ell_t - \ell_{t-1}) + (1-\beta^*) b_{t-1}$$
...
aus_economy <- global_economy %>% 
  filter(Code == "AUS")
fit <- aus_economy %>% 
  model(AAN = ETS(Population ~ error("A") + trend("A") + season("N")))
report(fit)
```
...
aus_economy %>%
  model(holt = ETS(Population ~ error("A") + trend("Ad") + season("N"))) %>% 
  forecast(h = 10) %>% 
  autoplot(aus_economy)
```
