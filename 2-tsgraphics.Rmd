...
ausgdp <- as_tsibble(expsmooth::ausgdp) %>% 
  rename(Time = index, GDP = value)
...
library(tsibble)
y <- tsibble(year = 2012:2016, y = c(123,39,78,52,110), index = year)
print(y)
```
...
```
ausgdp <- as_tsibble(x, index = Time)
```
...
```{r, echo=TRUE}
a10 %>% ggseasonplot(Scripts, year.labels=TRUE, year.labels.left=TRUE) +
  ylab("$ million") +
...
```{r}
beer %>% ggseasonplot(Production, year.labels=TRUE)
```
...
beer %>% gglagplot(Production, lags = 1:9)
```
...
```{r}
elec2 %>% ACF(value, lag.max=48) %>% autoplot()
```
...
```{r}
goog %>% ACF(Price, lag.max=100) %>% autoplot
```
