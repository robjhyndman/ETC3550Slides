...
```{r}
elecsales # not available in tsibbledata
```
...
```{r a10-plot, echo=TRUE}
a10 %>% autoplot(Cost/1e6) + ylab("$ million") + xlab("Year") +
  ggtitle("Antidiabetic drug sales")
```

## Your turn

- Create plots of the following time series: `fma::dole`, Bricks from `aus_production`, `pelt`, Google from `gafa_stock`
...
```{r, echo=TRUE}
a10 %>% ggseasonplot(Cost/1e6, labels = "both") +
  ylab("$ million") +
...
```{r, echo=TRUE}
a10 %>% ggsubseriesplot(Cost/1e6) + ylab("$ million") +
...
```{r}
beer %>% ggseasonplot(Beer, labels="right")
```

## Quarterly Australian Beer Production

```{r}
beer %>% ggsubseriesplot(Beer)
```
...
```{r, echo=FALSE}
wn %>% ACF(y, lag_max = 10) %>% knitr::kable(booktabs = TRUE)
```
...
in the state of Victoria, Australia, from January 1990 through December 1995.
(Source: Australian Bureau of Statistics.)\pause

  * Difficult to detect pattern in time plot.
  * ACF shows significant autocorrelation for the first 15 lags.
  * $r_{12}$ is relatively large.  This may indicate
