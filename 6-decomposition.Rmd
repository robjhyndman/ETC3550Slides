...
x %>%
  ...
  update_tsibble(key = Month, index = Year) %>%
  select(-Time) %>%
...
elecequip %>% SEATS(value) %>%
  autoplot() +
  ggtitle("X-13ARIMA-SEATS decomposition of electrical equipment index")
```
 ...
 * You must provide a method for forecasting the `seas_adjust` series.
