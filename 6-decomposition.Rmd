...
```{r elecequip3}
dcmp %>% ggsubseriesplot(season_year)
```
...
autoplot(elecequip, series="Data") +
  autolayer(dcmp, trend + remainder, series="Trend-cycle")
```
...
elecequip %>% autoplot(value, series="Data") +
  autolayer(dcmp, trend + remainder, series="Seasonally Adjusted")
```
...
x %>% 
  filter(year(Time) >= 2005) %>% 
  ggseasonplot(Employed, year.labels=TRUE) +
...
```{r x11, echo=FALSE, warning=FALSE, fig.width=8, fig.height=4, eval = FALSE}
library(seasonal)
fit <- seas(elecequip, x11="")
autoplot(fit) +
...
```{r seats, echo=FALSE, warning=FALSE, fig.width=8, fig.height=4, eval = FALSE}
library(seasonal)
fit <- seas(elecequip)
autoplot(fit) +
...
```{r seats-seasadj, echo=FALSE, warning=FALSE, eval = FALSE}
autoplot(elecequip, series="Data") +
...
```{r elecequip-seats, echo=TRUE, eval = FALSE}
...
```r
elecequip %>% STL(value ~ season(window = 5))

elecequip %>% STL(value ~ trend(window = 15) + season(window = "periodic"),
                  robust = TRUE)
```
...
dcmp <- elecequip %>% STL(value ~ trend(window = 13) + season(window = "periodic"))
dcmp %>% 
  model(NAIVE(seas_adjust)) %>%
  forecast() %>% 
...
dcmp_def <- dcmp_model(STL, value ~ trend(window = 13) + season(window = "periodic"),
                       NAIVE(seas_adjust))
elecequip %>%
  model(STLM = dcmp_def) %>% 
  forecast() %>%
