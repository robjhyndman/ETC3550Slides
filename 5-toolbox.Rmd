...
bricks %>%
  ...
  geom_point(data = slice(bricks, n()), aes(y=Bricks), colour = "blue") +
  ggtitle("Clay brick production in Australia")
```
...
bricks %>%
  ...
  forecast(h = "5 years") %>%
  autoplot(filter(bricks, year(Quarter) > 1990), level = NULL) +
  geom_point(data = slice(bricks, (n()-3):n()), aes(y=Bricks), colour = "blue") +
  ggtitle("Clay brick production in Australia")
```
...
aus_production %>%
  ...
  geom_line(data = slice(aus_production, range(cumsum(!is.na(Bricks)))),
            aes(y=Bricks), linetype = "dashed", colour = "blue") +
  ggtitle("Clay brick production in Australia")
```
...
slide_window <- slide(tourism_melb$Quarter, .size = 4) %>%
