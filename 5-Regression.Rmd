...
\[
  y_t = \beta_0 + \beta_1 x_{1,t} + \beta_2 x_{2,t} + \cdots + \beta_kx_{k,t} + e_t.
\]
\end{block}
...
```{r ConsInc, echo=TRUE, cache=TRUE}
...
```{r usestim, echo=TRUE}
fit.consMR <- tslm(Consumption ~
  Income + Production + Unemployment + Savings, data=uschange)
summary(fit.consMR)
```
...
\centering
$y_t = \beta_0 + \beta_1 t + \beta_2d_{1,t} + \beta_3 d_{2,t} + \beta_4 d_{3,t} + e_t$
\end{block}
...
* Ljung-Box test not recommended for regression models.
...
* This is done with a regression model with ARMA errors which will be covered in week 12.
...
\begin{block}{}
\[
\text{AIC}_{\text{C}} = \text{AIC} + \frac{2(k+2)(k+3)}{T-k-1}
\]
\end{block}
...
Let $\bm{y} = (y_1,\dots,y_T)'$, $\bm{e} = (e_1,\dots,e_T)'$, $\bm{\beta} = (\beta_0,\beta_1,\dots,\beta_k)'$ and
\[
\bm{X} = \begin{bmatrix}
...
\]
\end{block}
where $\bm{x}^*$ is a row vector containing the values of the regressors for the forecasts (in the same format as $\bm{X}$).
