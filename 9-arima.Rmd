...
```{r, echo=TRUE}
fit <- uschange %>% model(arima = ARIMA(Consumption ~ PDQ(0,0,0)))
report(fit)
```
...
internet %>% gg_tsdisplay(value)
```

## Choosing your own model

```{r, echo=TRUE, fig.height=4}
internet %>% gg_tsdisplay(difference(value))
```
...
```{r, echo=TRUE, fig.height=4}
fit %>% residuals %>% gg_tsdisplay(.resid, plot_type = "hist")
```
...
dcmp %>% gg_tsdisplay(difference(seas_adjust))
```
...
```{r, echo=TRUE, fig.height=4}
euretail %>% gg_tsdisplay(difference(value, 4))
```

## European quarterly retail trade

```{r, echo=TRUE, fig.height=3.8}
euretail %>% gg_tsdisplay(value %>% difference(4) %>% difference(1))
```
...
fit %>% residuals() %>% gg_tsdisplay(plot_type = "hist")
```
...
fit %>% residuals %>% gg_tsdisplay(.resid, lag_max=36, plot_type = "hist")
```
...
residuals(fit) %>% gg_tsdisplay(.resid, lag_max = 36, plot_type = "hist")
```
