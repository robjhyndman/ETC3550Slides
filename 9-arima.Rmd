...
gafa_stock %>% 
  ...
  ylab("Change in Google closing stock price") + xlab("Day")
```
...
usmelec %>% mutate(log_gen = log(Generation)) %>% 
  features(log_gen, list(unitroot_nsdiffs, stl_features))
```
...
usmelec %>% mutate(d_log_gen = difference(log(Generation), 12)) %>% 
  features(d_log_gen, list(unitroot_ndiffs, unitroot_kpss))
```
...
fit %>% forecast(h=10) %>% autoplot(tail(us_change,80))
```
...
```{r usconsumptionacf}
p1 <- us_change %>% ACF(Consumption) %>% autoplot()
p2 <- us_change %>% PACF(Consumption) %>% autoplot()
gridExtra::grid.arrange(p1,p2,nrow=1)
```
...
```{r, echo=TRUE, fig.height=4}
fit <- web_usage %>% model(arima = ARIMA(value ~ pdq(3, 1, 0)))
report(fit)
```
...
```{r, echo=TRUE, fig.height=4}
web_usage %>% model(ARIMA(value)) %>% report()
```
...
web_usage %>% model(ARIMA(value, stepwise = FALSE, approximation = FALSE)) %>% report()
```
...
```{r}
augment(fit) %>% features(.resid, ljung_box, lag = 24, dof = 4)
```
...
```{r, echo = TRUE}
augment(fit) %>% features(.resid, ljung_box, lag = 8, dof = 2)
```
...
```{r, echo=TRUE}
augment(fit) %>% features(.resid, ljung_box, lag = 8, dof = 4)
```
...
augment(fit) %>% features(.resid, ljung_box, lag = 36, dof = 6)
```
...
augment(fit) %>% features(.resid, ljung_box, lag = 36, dof = 5)
```
...
augment(fit) %>% features(.resid, ljung_box, lag = 36, dof = 9)
```
