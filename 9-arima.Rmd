...
### ARIMA(1,0,3) model:
\centerline{$
  y_t = c + `r format(tidy(fit)$estimate[1], nsmall=3, digits=3)`y_{t-1}
          ...
          + \varepsilon_{t},
$}
where $c= `r format(tidy(fit)$estimate[5], nsmall=3, digits=3)` \times (1 - `r format(tidy(fit)$estimate[1], nsmall=3, digits=3)`) = `r format(tidy(fit)$estimate[5] * (1-tidy(fit)$estimate[1]), nsmall=3, digits=3)`$
and $\varepsilon_t$ is white noise with a standard deviation of $`r format(glance(fit)$sigma, nsmall=3, digits=3)` = \sqrt{`r format(glance(fit)$sigma^2, nsmall=3, digits=3)`}$.
...
```{r, echo = TRUE}
us_change %>% autoplot(difference(Consumption)) +
  labs(x = "Year", y = "US consumption", 
...
web_usage %>% model(ARIMA(value, stepwise = FALSE,
...
fit <- dcmp %>% 
  model(
    arima = ARIMA(seas_adjust ~ pdq(3,1,1) + PDQ(0,0,0))
  )
report(fit)
```
...
euretail %>% model(ARIMA(value, stepwise = FALSE,
  approximation = FALSE)) %>% report()
```
...
fit <- h02 %>% 
  model(best = ARIMA(log(Cost) ~ pdq(3,0,1) + PDQ(0,1,2)))
report(fit)
```
...
fit <- h02 %>% model(best = ARIMA(log(Cost), stepwise = FALSE,
  approximation = FALSE, order_constraint = p + q + P + Q <= 9))
report(fit)
```
